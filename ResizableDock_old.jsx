...
function DockTabFallback() {
  return <WidgetEmpty message="Loading tabÔÇª" />;
}
...
// ÔöÇÔöÇ Tiny formatters ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const priceDecimals = (sym, price) =>
  (sym?.includes('XRP') || sym?.includes('ADA') || sym?.includes('DOGE') || (price != null && price < 2.0)) ? 4 : 2;

const fmtP = (n, d = 2) =>
  n == null ? 'ÔÇö' : Number(n).toLocaleString('en-US', { minimumFractionDigits: d, maximumFractionDigits: d });
...
/** Binance maps USD ÔåÆ USDT; skip duplicate row/total when values match. */
...
// ÔöÇÔöÇ Position Row ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇ Positions Tab ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        <div className="dock-panel-tab__toolbar-lead">
          ...
          <div className="dock-panel-tab__toolbar-copy">
            ...
            <span className="dock-panel-tab__toolbar-subtitle num-mono">
              ...
              {entries.length > 0 && (
                <> ┬À {stats.longCount}L / {stats.shortCount}S</>
              )}
            </span>
          </div>
        </div>
      ...
      {entries.length === 0 ? (
        ...
        <>
          ...
          <footer className="dock-panel-tab__footer">
            <span>
              {entries.length} open ┬À {stats.longCount} long ┬À {stats.shortCount} short
            </span>
...
// ÔöÇÔöÇ Orders Tab ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function OrdersTab() {
    ...
    <div className="dock-panel-tab">
        ...
        <div className="dock-panel-tab__toolbar-lead">
          ...
          <div className="dock-panel-tab__toolbar-copy">
            ...
            <span className="dock-panel-tab__toolbar-subtitle num-mono">
              ...
              {active.length > 0 && (
                <> ┬À {stats.buyCount}B / {stats.sellCount}S</>
              )}
            </span>
          </div>
        </div>
      ...
      {active.length === 0 ? (
        ...
        <>
          ...
          <footer className="dock-panel-tab__footer">
            <span>
              {active.length} pending ┬À {stats.buyCount} buy ┬À {stats.sellCount} sell
            </span>
            <span className="dock-panel-tab__footer-highlight">
              Total notional:{' '}
...
// ÔöÇÔöÇ Balances Tab ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function BalancesTab() {
  ...
  return (
    <div className="dock-panel-tab">
      ...
      {rows.length === 0 ? (
        <div className="dock-panel-tab__empty">
          <WidgetEmpty icon={Landmark} message="Loading balancesÔÇª" />
        </div>
      ) : (
        <>
          <div className="dock-panel-tab__table-wrap scroll-panel-y scroll-panel-y-0">
                ...
                {rows.map(({ asset, bal, avail, usdValue, isQuote }) => {
                  ...
                  return (
                    <DataTableRow key={asset} rowVariant="dock" deferred>
                      ...
                      <DataTableCell numeric align="right" className="text-secondary-foreground">
                        {usdValue != null ? `$${fmtP(usdValue)}` : 'ÔÇö'}
                      </DataTableCell>
                    </DataTableRow>
                  );
                })}
          ...
          <footer className="dock-panel-tab__footer">
            <span>{rows.length} assets ┬À cash + holdings</span>
            <span className="dock-panel-tab__footer-highlight">
              Cash available:{' '}
              <span className="num-mono font-bold">${fmtP(stats.cashAvailable)}</span>
              {stats.cashLocked > 0 && (
                <span className="text-muted-foreground">
                  {' '}┬À locked ${fmtP(stats.cashLocked)}
                </span>
              )}
              {stats.holdingsUsd > 0 && (
                <span className="text-muted-foreground">
                  {' '}┬À holdings ${fmtP(stats.holdingsUsd)}
                </span>
              )}
              {' '}┬À total ${fmtP(stats.totalEquity)}
            </span>
          </footer>
        </>
      )}
...
// ÔöÇÔöÇ Algo Bot Tab ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
export function AlgoTab({ hideToolbar = false }) {
  ...
  const handleRunBacktest = async () => {
    ...
    setBacktestProgress({ pct: 0, phase: 'resolve', message: 'Starting backtestÔÇª' });
    ...
    scheduleBacktestClientTimeout({
      ...
      onTimeout: (timeoutMs) => {
        if (useStore.getState().backtestRunning) {
          ...
          toast.error(
            backtestReasoning
              ? `Backtest timed out after ${formatBacktestTimeoutLabel(timeoutMs)} ÔÇö increase VITE_BACKTEST_REASONING_TIMEOUT_MS, reduce days, or lower BACKTEST_REASONING_MAX_TRADES`
              : `Backtest timed out after ${formatBacktestTimeoutLabel(timeoutMs)} ÔÇö try a shorter range or increase VITE_BACKTEST_TIMEOUT_MS`,
          );
        }
      },
    });
          ...
          <div className="algo-tab__toolbar-lead">
            ...
            <div className="algo-tab__toolbar-copy">
              ...
              <span className="algo-tab__toolbar-subtitle num-mono">
                {runningCount} running ┬À {activeBots.length} bot{activeBots.length === 1 ? '' : 's'} ┬À {activeSymbol}
              </span>
            </div>
          </div>
      ...
      {isLive && allowLiveBots && (
        <Alert className="algo-tab__banner border-trading-up/30 bg-trading-up/5 xl:col-span-3">
          ...
          <AlertDescription className="text-xs leading-relaxed">
            ...
            {massiveLive
              ? ' ÔÇö instant fills at live prices (no broker routing). 1m BAR_CLOSE via feed bar hooks; higher timeframes via native REST; TICK bots on price updates.'
              : ` on ${terminalMode}`}
            {distributed ? ` ┬À role=${terminalRole} (distributed via Redis)` : ''}.
            {!massiveLive && (
              <>
                {' '}Indicator warm-up uses archive when buffer &lt; {botMinCandles} bars.
                Signals fire on closed {formatBarTimeframeLabel(botTimeframe)} bars ÔÇö do not resend ambiguous orders.
              </>
            )}
            {massiveLive && (
              <>
        ...
        <Alert className="algo-tab__banner border-trading-warn/40 bg-trading-warn/5 xl:col-span-3">
          ...
          <AlertDescription className="flex flex-wrap items-center gap-2 text-xs leading-relaxed">
            <span>
              ...
              {' '}need review ÔÇö confirm filled or dismiss in Reconcile (do not resend).
            </span>
            <Button
              variant="outline"
              size="sm"
      ...
      <section className="algo-tab__panel algo-tab__panel--deploy">
        <header className="algo-tab__panel-header">
          <div className="algo-tab__panel-heading">
            ...
            <span className="algo-tab__panel-subtitle">Strategy ┬À caps ┬À backtest</span>
          </div>
        </header>
        <div className="algo-tab__scroll scroll-panel-y scroll-panel-y-0 algo-tab__deploy-body" data-tour="algo-deploy">
          <div className="algo-deploy-fields">
            ...
            <div className="algo-deploy-field">
              ...
              >
                ...
                <SelectContent position="popper">
                  <SelectItem value="BAR_CLOSE" className="text-xs">Bar Close ÔÇö indicator signals on bar close</SelectItem>
                  <SelectItem value="TICK" className="text-xs">Tick ÔÇö sub-minute microstructure</SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="algo-deploy-field">
              ...
              <span className="algo-field-hint">
                Strategy evaluates on closed {formatBarTimeframeLabel(botTimeframe)} candles ÔÇö same resolution as backtest below.
              </span>
            </div>
            )}
            ...
            <div className="algo-deploy-field">
              ...
              >
                ...
                <SelectContent position="popper">
                  ...
                  <SelectItem value="none" className="text-xs">None ÔÇö trailing stop only</SelectItem>
                </SelectContent>
              </Select>
              ...
              <div className="algo-deploy-field space-y-2">
                ...
                <label className="flex cursor-pointer items-center gap-2 text-xs text-muted-foreground">
                  <input
                    ...
                    className="accent-primary"
                  />
                  Require trend alignment (BUY ÔëÑ +1, SELL Ôëñ ÔêÆ1)
                </label>
                <label className="flex cursor-pointer items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
            ...
            <div className="algo-deploy-field">
              ...
              <span className="algo-field-hint">
                {massiveLive
                  ? 'Shared with deploy timeframe ÔÇö backtest uses archive; live Massive bots use native HT REST where available.'
                  : 'Shared with deploy timeframe ÔÇö resampled from archived 1m data.'}
              </span>
            </div>
            ...
            <label className="algo-backtest-oos flex items-center gap-2 text-[0.62rem] text-muted-foreground cursor-pointer">
              ...
              />
              Hold-out test (last 30%) ÔÇö test on last 30% of range only
            </label>
            ...
            <label className="flex items-center gap-2 text-[0.62rem] text-muted-foreground cursor-pointer">
              ...
              />
              Portfolio backtest ÔÇö run same strategy on top 5 watchlist symbols
            </label>
            ...
            {agentLlmAvailable ? (
              ...
              <p className="text-[0.62rem] text-muted-foreground">
                LLM unavailable ÔÇö start Ollama or configure OpenRouter to enable post-backtest trade explanations.
              </p>
            )}
        ...
        <footer className="algo-tab__panel-footer algo-deploy-actions">
          <div className="algo-deploy-actions__rail">
            ...
            >
              ...
              {backtestRunning ? 'RUNNINGÔÇª' : 'BACKTEST'}
            </Button>
            <Button
              variant="ghost"
              size="sm"
        ...
        <DialogContent className="algo-dialog sm:max-w-md" overlayClassName="admin-panel-overlay">
          ...
          <div className="algo-dialog-summary">
            ...
            <div>
              ...
              <strong>
                SL {botConfig?.trailing_stop_percent ?? botConfig?.stop_loss_percent ?? 'ÔÇö'}%
                {' ┬À '}
                {botConfig?.tp_mode === 'none'
                  ? 'no TP'
                  : botConfig?.tp_mode === 'strategy'
                    ? 'strategy target'
                    : `${botConfig?.take_profit_percent ?? 'ÔÇö'}% TP`}
              </strong>
            </div>
      ...
      <section className="algo-tab__panel algo-tab__panel--bots">
        <header className="algo-tab__panel-header">
          <div className="algo-tab__panel-heading">
            ...
            <span className="algo-tab__panel-subtitle">Pause ┬À resume ┬À stop ┬À details</span>
          </div>
          <div className="algo-tab__panel-actions">
            {activeBots.length > 0 && (
              <span className="algo-bots-scroll-hint">Scroll Ôåö</span>
            )}
            {activeBots.length > 0 && (
              <Button
                variant="outline"
...
// ÔöÇÔöÇ Global deploy dialog (optimizer + cross-tab pendingDeploy) ÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇ Main ResizableDock ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
export default function ResizableDock({ setDockHeight: setParentDockHeight, initialDockHeight }) {
  ...
  const TABS = useMemo(() => {
    const tabs = [
      ...
      { id: 'scanner',   label: 'Scanner',   icon: Radar,    badge: scanBadge, group: 'intelligence', hint: 'Quick peek ÔÇö open Hub (ÔîÿI) for full scanner workspace' },
      { id: 'analyst',   label: 'Analyst',   icon: Brain,    badge: analystBadge, group: 'intelligence', hint: 'Quick peek ÔÇö open Hub (ÔîÿI) for full analyst history' },
  ...
  if (dockCollapsed) {
    ...
    return (
      <>
        ...
        role="button"
        tabIndex={0}
        aria-label={`Expand dock ÔÇö ${groupLabel}, ${collapsedTab?.label ?? 'tab'}`}
        title="Expand dock"
        onClick={expandDock}
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') {
          ...
          <div className="dock-tab-bar">
            ...
            {activeGroup === 'portfolio' && (
              <Button
                variant="outline"
                size="sm"
                title="Open Portfolio Dashboard ÔÇö equity, allocation, risk analytics"
            ...
            {activeGroup === 'intelligence' && (
              <Button
                variant="outline"
                size="sm"
                title="Open Insights Hub ÔÇö resizable scanner + analyst workspace (ÔîÿI)"
